...
|------|------|-----------------------------|--------------|
| **1** | Import Libraries | Required to work with data and handle missing values | ```import pandas as pd```| 
|       |                   |                                                     |  ```import numpy as np``` |                                                                          
| **2** | Load CSV File (Local) | Load dataset from your computer | ```df = pd.read_csv("file_path.csv")``` |
| **3** | Load CSV File (URL) | Load dataset directly from the web (works outside JupyterLite) | ```df = pd.read_csv("https://example.com/data.csv")``` |
| **4** | Load File Without Header | When the file has no column names | ```df = pd.read_csv("file.csv", header=None)``` |
| **5** | View Top Rows | Helps inspect structure and values | ```df.head()``` |
| **6** | View Bottom Rows | Check end of dataset for missing/irregular values | ```df.tail(10)``` |
...
| **13** | Find Missing Values | Count how many NaNs exist in each column | ```df.isnull().sum()``` |
| **14** | Drop Missing Target Values | Removes rows where key values (e.g., price) are missing | ```df.dropna(subset=["price"], inplace=True)``` |
| **15** | Reset Index After Drop | Avoids duplicate index numbers | ```df.reset_index(drop=True, inplace=True)``` |
| **16** | Statistical Summary (Numeric) | Provides mean, median, std, etc. for numerical data | ```df.describe()``` |
| **17** | Summary for All Columns | Includes object-type columns as well | ```df.describe(include="all")``` |
| **18** | Save Cleaned File | Export cleaned dataset to a CSV file | ```df.to_csv("cleaned_dataset.csv", index=False)``` |
